...
$result = Get-Content $testFile | .\target\spring-boot-cli.exe -l
if ($result -match "^\d+$") { $passedTests++ }

Write-Host "`n7. Testing -w (word count) with stdin:"
$result = Get-Content $testFile | .\target\spring-boot-cli.exe -w
if ($result -match "^\d+$") { $passedTests++ }

Write-Host "`n8. Testing -c (byte count) with stdin:"
$result = Get-Content $testFile | .\target\spring-boot-cli.exe -c
if ($result -match "^\d+$") { $passedTests++ }

Write-Host "`n9. Testing -m (character count) with stdin:"
$result = Get-Content $testFile | .\target\spring-boot-cli.exe -m
if ($result -match "^\d+$") { $passedTests++ }

Write-Host "`n10. Testing default (all counts) with stdin:"
$result = Get-Content $testFile | .\target\spring-boot-cli.exe
